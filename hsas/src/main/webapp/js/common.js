 ...
 */
function generateLink(name, id, type){
	var type = type || 0;
	var node = " <div class='selected-link' type =" + type + "><span class='label label-primary' id= " + id + ">" + name + 
	"</span><div class='delete-link'><i class='fa fa-times'></i></div></div> ";
	return $(node);
}
// 重写alert函数
alert = function(str){
	dialog({
	    title: '提示',
	    content: "<span style='word-break: break-word'>" + str  + "</span>",
	    okValue: '确定',
	    width:250,
	    ok: function () {
	        return true;
