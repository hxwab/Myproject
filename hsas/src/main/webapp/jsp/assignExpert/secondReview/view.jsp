...
<html>
    ...
    <body>
			    ...
			    <div class="col-xs-12 main-content">
					...
					<textarea id = "view_template" style="display:none">
						  	...
						  	<div class="panel-body">
							    <table class="table table-striped view-table">
							      <tbody>
									...
									<tr>
										<td width = "50" class = "text-right"><span class="glyphicon glyphicon-triangle-right" aria-hidden="true"></span></td>
										<td width = "100" class = "text-right">备注：</td>
						...
						<textarea id = "expert_list_template" style="display:none">
						 	<div class="btn-group pull-right view-controler" role="group" aria-label="...">
								<button type="button" class="btn  btn-default add-expert">添加专家</button>
								<button type="button" class="btn  btn-default set-leader">指定组长</button>
							...
							<table class="table table-bordered" style="font-size:14px;">
						      <thead>
						        <tr>
						          ...
						          <th>所属机构</th>
						     	  ...
						     	  <th  width = "110">是否为组长</th>
						     	  <th width = "120">当年是否有成果</th>
						        </tr>
						      </thead>
						      <tbody>
						      {for item in assignedExpert}
						        <tr>
						          ...
						          <td><a id="${item[0] }" title="点击分配成果" class="assign-product">${item[1] }</a></td>
						          <td>${item[2] }</td>
						          <td>${item[3] }</td>
						          <td>${item[4] }</td>
						          <td>{if item[5] == 1} <span style="color:green"><i class="fa fa-users" ></i>&nbsp;组长</span> {else} <a class="change-status"><i class="fa fa-user"></i>&nbsp;组员{/if}</a></td>
						          <td class="text-center">{if item[6] == 1}<span class="label label-success label-custom">第一作者</span> {elseif item[6] == 2}<span class="label label-info label-custom">第二作者</span> {else}<span class="label label-default label-custom">无</span>{/if}</td>
						        </tr>
						       {forelse}
						          <td colspan = "8" class="text-center">还未分配专家！</td>
						        {/for}
						      </tbody>
						    </table>
						</textarea>
											...
											<tbody>
												<tr>
													<td align="right">
														...
														<input  name="reviewType" type="hidden"  value="1">
													</td>
										...
										<table id="list_table" class="table table-striped table-bordered dataTable no-footer" width="100%" border="0" cellspacing="0" cellpadding="0">
											<thead id="list_head">
												<tr class="table_title_tr">
													...
													<td width="100" ><a id="sortcolumn7" href="" class="{if sortColumn == 7}{if sortColumnLabel == 1}up_css{else}down_css{/if}{/if}" title="点击按账号名称排序">初评得分</a></td>
												</tr>
											</thead>
											<tbody>
												{for item in root}
												<tr>
													...
													<td >${item.laData[1]}</td>
													<td class="table_txt_td">a${item.laData[2]}</td>
													<td >${item.laData[3]}</td>
													<td >${item.laData[4]}</td>
													<td >${item.laData[5]}</td>
													<td >${item.laData[6]}</td>
		...
		<script>
		    seajs.use("js/assignExpert/secondReview/view.js", function(view) {
		    	view.init(); 
		    });
		</script>
	</body>
