...
public class MyUserDeitailsService implements UserDetailsService {
	...
	public UserDetails loadUserByUsername(String username)
		...
		if (account == null) {// 如果账号不存在，则抛异常
			...
			for(String right :userRight){
				
				GrantedAuthority  grantedAuthority = new GrantedAuthorityImpl(right);
				authorities.add(grantedAuthority);
			}
			
			
			UserDetails userDetails = new User(username,  account.getPassword(), true, true, true, true,authorities );
			return userDetails; */
