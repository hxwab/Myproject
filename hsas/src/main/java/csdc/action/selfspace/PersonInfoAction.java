...
public class PersonInfoAction extends BaseAction {
	...
	public String modify() {
		Person nperson = account.getPerson();
		Account nAccount = account;
		LoginInfo  loginer = (LoginInfo) ActionContext.getContext().getSession().get(GlobalInfo.LOGINER);
		account = loginer.getAccount();
		Person  operson = account.getPerson();
		
		Agency agency = personInfoService.getAgency(nAccount.getAgency().getId());
		nperson.setAgency(agency);
		nperson.setAgencyName(agency.getName());
		if(nperson.getAgencyName()!=operson.getAgencyName()){
			account.setAgency(agency);
			personInfoService.modifyAccountInfo(account);
		}
		
		personInfoService.modifyPersonInfo(nperson, operson);
		jsonMap.put("tag", "1");
		return SUCCESS;
	}
