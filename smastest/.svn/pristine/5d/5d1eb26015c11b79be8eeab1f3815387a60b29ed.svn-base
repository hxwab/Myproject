...
public class ExpertTitleStatisticAwardGroupAction extends ReviewStatisticAction {
	private static final long serialVersionUID = 1L;
	private static final String HQL4AWARDGROUP = "select ag.id, ag.name, count(distinct aagr.reviewer.id), 'experttitle' from AwardApplicationGroupReview aggr, Expert e left join aggr.awardGroup ag where aagr.type = 'moeSocial' and e.id = aagr.reviewer.id and aagr.year = :defaultYear ";
	private static final String HQLG = " group by ag.id, ag.name ";
	private static final String PAGENAME = "statisticAGETPage";
	private static final String PAGE_BUFFER_ID = "ag.id";//缓存id
	private static final String column[] = {
		"ag.name",
		"count(distinct aagr.reviewer.id)"
	};// 排序用的列
	...
	public void pageListDealWith() {
		...
		for (Object p : pageList) {
			...
			List<Object[]> expertTitles = baseService.query("select e.isKeyProjectDirector," +//是否重大项目负责人
					...
					" from AwardApplicationGroupReview aagr, Expert e where e.id = aagr.reviewer.id and aagr.awardGroup.id = ?", item[0].toString());
