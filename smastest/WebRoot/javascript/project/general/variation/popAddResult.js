...
$(document).ready(function(){
	...
	*/
	var sortNum = function() {
		$(".table_valid tbody tr").each(function(index, element) {
			$(this).find(".num").html(index+1);
		});
		$(".varMemberTr",$("#varMembers")).each(function(key1,value){
			$(":input[name^=newMember]",value).each(function(key2, value){
	...
	*/
	...
	$(".add_member").live("click", function() {
		var template = $("#varMembersTempl tr").clone();
		var member = $(".varMemberTb .table_valid tbody");
		template.appendTo(member);
		sortNum();
	});
	...
	*/
	...
	$(".moreDetail").live("click", function(){
		...
		popMemberDetail({
			title: "变更人员详情",
			src: "project/general/variation/toPopMemberDetail.action",
			inData: {
				...
				foreign: $(":input[name*='foreign']", onTr).val(),
				education: $(":input[name*='education']", onTr).val(),
				degree: $(":input[name*='degree']", onTr).val(),
				title: $(":input[name*='title']", onTr).val(),
				agencyName: $(":input[name*='agencyName']", onTr).val(),
				divisionName: $(":input[name*='divisionName']", onTr).val(),
				job: $(":input[name*='job']", onTr).val(),
				email: $(":input[name*='email']", onTr).val(),
				mobile: $(":input[name*='mobile']", onTr).val(),
				phone: $(":input[name*='phone']", onTr).val(),
				postcode: $(":input[name*='postcode']", onTr).val(),
				address: $(":input[name*='address']", onTr).val(),
				isDirector: $(":input[name*='isDirector']", onTr).val()
			},
			callBack: function(data) {
				$(":input[name*='memberType']", onTr).val(data.memberType);
				$(":input[name*='memberName']", onTr).val(data.memberName);
				$(":input[name*='gender']", onTr).val(data.gender);
				$(":input[name*='birthday']", onTr).val(data.birthday);
				$(":input[name*='idcardType']", onTr).val(data.idcardType);
				$(":input[name*='idcardNumber']", onTr).val(data.idcardNumber);
				$(":input[name*='foreign']", onTr).val(data.foreign);
				$(":input[name*='education']", onTr).val(data.education);
				$(":input[name*='degree']", onTr).val(data.degree);
				$(":input[name*='title']", onTr).val(data.title);
				$(":input[name*='agencyName']", onTr).val(data.agencyName);
				$(":input[name*='divisionName']", onTr).val(data.divisionName);
				$(":input[name*='job']", onTr).val(data.job);
				$(":input[name*='email']", onTr).val(data.email);
				$(":input[name*='mobile']", onTr).val(data.mobile);
				$(":input[name*='phone']", onTr).val(data.phone);
				$(":input[name*='postcode']", onTr).val(data.postcode);
				$(":input[name*='address']", onTr).val(data.address);
				$(":input[name*='isDirector']", onTr).val(data.isDirector);
			}
		});
	});
