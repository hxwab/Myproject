...
<html>
		...
		<div class="row mySlide">
			...
			<div class="col-xs-12">
				...
				<div id="simple_search" style="display: none !important;">
					<form id="search" name="search" action="jsp/system/dataDic/discipline/list.json" method="post"
						  class="form-inline">
					...
					<table id="list_table" class="table table-striped table-bordered dataTable no-footer" width="100%" border="0" cellspacing="0" cellpadding="0">
						<thead id="list_head">
							<tr class="table_title_tr">
								...
								<td width="150" ><a id="sortcolumn1" href="" class="{if sortColumn == 1}{if sortColumnLabel == 1}up_css{else}down_css{/if}{/if}"  title="点击按奖励数量排序">学科门类数量</a></td>
								<td width="400" ><a id="sortcolumn2" href="" class="{if sortColumn == 2}{if sortColumnLabel == 1}up_css{else}down_css{/if}{/if}"  title="点击按初评专家排序">学科门类</a></td>
							</tr>
						</thead>
						<tbody>
							{for item in root}
	...
	<script>
		seajs.use("js/system/dataDic/discipline/list.js", function(list) {
			list.init(); 
		});
	</script>
</html>