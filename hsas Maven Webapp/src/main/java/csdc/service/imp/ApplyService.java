...
public class ApplyService extends BaseService implements IApplyService {
	...
	public String addApplyInfos(List<Person> person, Product product,Agency agency,Map map) {
		...
		//Agency agen = getAgency(agencyName);
		...
		product.setAgency(agencys.get(0));
		product.setAgencyName(agencys.get(0).getName());
	...
	private String parseDisciplineName(String disciplineName){
		
		String str="";
		switch (Integer.parseInt(disciplineName)) {
		case 0:
			str="马克思主义与党建（社科）";
			break;
		case 1:
