...
public class RoleService extends BaseService implements IRoleService {
	...
	public List<String> getRightNodeValue(String roleId) {
		...
		if (roleId == null) {// 未指定角色，则读取所有权限节点
			...
			Map map = new HashMap();
			map.put("roleId", roleId);
			re = dao.query("select r.nodeValue from Right r left join r.roleRights rR where rR.role.id = :roleId", map);
		}
		return re;
	}
	...
	private Right getRightByNodeValue(String nodeValue) {
		...
		try {
			list = dao.query("select r from Right r where r.nodeValue = :nodeValue  ", map);
		} catch (Exception e) {
